...
There is also a forecast compatibility gap in the pinned SoL-Pi version: its `session_compact` handler clears carried cache debt when Pi reports `fromExtension`. Pi sets this flag when a compaction hook supplies the summary, which is OM's normal path. Consequently, subsequent decisions do not carry forward that compaction's estimated cache rebuild debt. This experiment composes the upstream mechanisms as they are; it has not fully adapted the economic forecast to OM summaries. See the [pinned handler](https://github.com/NVlabs/SoL-Pi/blob/d7ecfc089944f0d04b80122a0a9a6ca0d786f3d0/src/sol-pi/extensions/online-context-compact/extension.ts).
